...
Version 2.2 introduces a purpose-built Home experience: original memory-trail artwork, a denser no-scroll phone layout, tactile item shortcuts, a focused custom-item sheet, and a useful recent-find card that opens the exact history entry. Version 2.2.1 refines the artwork motion into one calm, session-only search demonstration that briefly checks the remote and kitchen before finding the keys. It retains the saved home spots, learned search patterns, local backup and restore, and update-ready notices added in 2.1.
